  ...
  return (
    <div className="screen">
      ...
      <main className="list">
        ...
        <section>
          <h2 className="section-title">🔴 긴급 ({urgent.length})</h2>
          {urgent.length === 0 && <p className="empty">없음</p>}
          {urgent.map((j) => <JobCard key={j.villaId} job={j} />)}
        </section>

        <section>
          <h2 className="section-title">🟡 권장 ({recommended.length})</h2>
          {recommended.length === 0 && <p className="empty">없음</p>}
          {recommended.map((j) => <JobCard key={j.villaId} job={j} />)}
        </section>

        <section>
          <h2 className="section-title">⚪ 유예 가능 ({deferrable.length + acceptedJobs.length})</h2>
        ...
        {bundles.length > 0 && (
          <section>
            <h2 className="section-title">💡 묶어가기 추천</h2>
            {bundles.map((rec) => (
              <div key={rec.job.villaId} className="bundle-card">
                <div className="bundle-head">
                  <strong>💡 {rec.job.villa.name}</strong>
                  <span className="villa-meta">({rec.job.reason.split("—")[1]?.trim() ?? rec.job.reason})</span>
                </div>
                <ul className="bundle-numbers">
                  <li>오늘 경로에 끼우면 이동 <b>+{rec.detour}분</b></li>
                  <li>따로 가면 왕복 <b>{rec.solo}분</b></li>
                  <li>→ <b className="saving">{rec.saving}분 절약</b></li>
                </ul>
      ...
      <footer className="bottom-bar">
        <button className="primary-btn" onClick={onShowRoute}>
